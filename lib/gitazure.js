...
function GitHook(server, repoUrl, branch, config) {
  this.config = config || {};
  this.config.npmInstall = typeof this.config.npmInstall == 'undefined' ? true : false;
  this.config.npmPrune = typeof this.config.npmPrune == 'undefined' ? true : false;
  this.server = server;
  this.branch = branch;
        ...
        if (body.substr(0, 8) == 'payload=') {
          try {
            ...
            if (payload.repository.url == repoUrl) {
              if (self.isCorrectBranch(payload)) self.processPayload(payload, branch, res);
...
module.exports = {
  listen: function(server, repoUrl, branch, config) {
    branch = branch || 'master';
    return new GitHook(server, repoUrl, branch, config);
  }
};

GitHook.prototype.processPayload = function(payload, branch, res) {
  console.info('GitHook is pulling from the repository');
  var self = this;
  this.gitPull(branch, function(error) {
    if (error) {
      console.error(error);
      res.end();
    }
...
GitHook.prototype.gitPull = function(branch, cb) {
  var prevHome = process.env.appdata;
  process.env.home = 'bin';
  var util  = require('util')
    , spawn = require('child_process').spawn
    , git = spawn('git', ['pull', 'origin', branch])
    , output = '';
...
GitHook.prototype.isCorrectBranch = function(payload) {
  return payload.ref == 'refs/heads/' + this.branch;
}
